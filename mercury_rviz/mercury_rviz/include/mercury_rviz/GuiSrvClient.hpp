...
//
// mercury_rviz srv client utility. Handles calls to services without blocking the ui with feedback to the user.
//
...
public:
  ...
  typedef std::function<void(const std::string &, typename rclcpp::Client<SrvType>::SharedResponse)>
    ResponseFunc;

  GuiSrvClient(
    rclcpp::Node::SharedPtr node, const std::string & srv_name,
    StatusUpdateFunc status_update_callback, ResponseFunc response_callback)
  : node(node), updateStatus(status_update_callback), respond(response_callback), srvReqId(-1)
  {
  ...
  void callService(typename SrvType::Request::SharedPtr request)
  {
    ...
    updateStatus(
      QString("Making call to service %1").arg(QString::fromStdString(srvName)), "000000");
    auto future = client->async_send_request(request);
    srvReqId = future.request_id;
    sharedFuture = future.share();
    //this is how we wait for a service result
    QTimer::singleShot(250, [this]() { this->waitForService(); });
