#include "mercury_rviz/ControlPanel.hpp"
...
#define EXPECTED_CONTROLLER_DIAG_KEYS \
    5 // getting unreliable message size data, so this is hard coded :( - this should be the number
      // of keys in the message not the number that should be accessed
...
const std::string get_hostname() {
    ...
    if (hostnameInternal.length() >= MAX_HOST_LEN) {
        ...
        std::cerr << "Failed to discover system hostname, falling back to default, "
                  << hostnameInternal;
    } else {
...
ControlPanel::ControlPanel(QWidget * parent) : rviz_common::Panel(parent) {
    ...
    ctrlMode = mercury_rviz::ControlPanel::control_modes::DISABLED;
...
void ControlPanel::onInitialize() {
    ...
    // mode seting buttons
    connect(uiPanel->ctrlModePos, &QPushButton::clicked, [this](void) {
        switchMode(mercury_rviz::ControlPanel::control_modes::POSITION);
    });
    connect(uiPanel->ctrlModeVel, &QPushButton::clicked, [this](void) {
        switchMode(mercury_rviz::ControlPanel::control_modes::VELOCITY);
    });
    connect(uiPanel->ctrlModeFFD, &QPushButton::clicked, [this](void) {
        switchMode(mercury_rviz::ControlPanel::control_modes::FEEDFORWARD);
    });
    connect(uiPanel->ctrlModeTele, &QPushButton::clicked, [this](void) {
        switchMode(mercury_rviz::ControlPanel::control_modes::TELEOP);
    });
    ...
    connect(uiPanel->CtrlSendCmd, &QPushButton::clicked, this, [this]() {
        ControlPanel::handleCommand(true);
    });

    // parameter reload buttons
    connect(
        uiPanel->reloadController, &QPushButton::clicked, this,
        &ControlPanel::handleReloadController);

    // connect simulator apply
    connect(
        uiPanel->simulation_apply, &QPushButton::clicked, this,
        &ControlPanel::simulator_apply_clickec);
...
void ControlPanel::load(const rviz_common::Config & config) {
    ...
    connect(uiTimer, &QTimer::timeout, [this](void) { refreshUI(); });
    ...
    selectPoseSub = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "goal_pose", rclcpp::SystemDefaultsQoS(), std::bind(&ControlPanel::selectedPose, this, _1));

    // setup the ROS topics that depend on namespace
    // make publishers
    auxPub = node->create_publisher<std_msgs::msg::Bool>(
        robot_ns + "/state/aux", rclcpp::SystemDefaultsQoS());
    killStatePub = node->create_publisher<mercury_msgs::msg::KillSwitchReport>(
        robot_ns + "/command/software_kill", rclcpp::SystemDefaultsQoS());

    // controller setpoint publishers
    ctrlCmdLinPub = node->create_publisher<mercury_msgs::msg::ControllerCommand>(
        robot_ns + "/controller/linear", rclcpp::SystemDefaultsQoS());
    ctrlCmdAngPub = node->create_publisher<mercury_msgs::msg::ControllerCommand>(
        robot_ns + "/controller/angular", rclcpp::SystemDefaultsQoS());
    ...
    reloadCompleteClient = node->create_client<Trigger>(
        robot_ns + "/controller_overseer/update_complete_controller_params");

    // interactive marker for setpoint
    setptServer = std::make_shared<interactive_markers::InteractiveMarkerServer>(
    ...
    setPointPubTimer = node->create_wall_timer(
        SETPOINT_REPUB_PERIOD, std::bind(&ControlPanel::pubCurrentSetpoint, this));
...
void ControlPanel::pubCurrentSetpoint() {
    if (ctrlMode == mercury_rviz::ControlPanel::control_modes::POSITION) {
...
void ControlPanel::handleDisable() {
    ...
    switchMode(mercury_rviz::ControlPanel::control_modes::DISABLED, true);

    // disable controllers
    mercury_msgs::msg::ControllerCommand disableCmd;
    disableCmd.mode = mercury_rviz::ControlPanel::control_modes::DISABLED;
...
void ControlPanel::switchMode(uint8_t mode, bool override) {
    ...
    if (vehicleEnabled || override) {
        ...
        switch (ctrlMode) {
            case mercury_rviz::ControlPanel::control_modes::POSITION:
                ...
                break;
            case mercury_rviz::ControlPanel::control_modes::VELOCITY:
                uiPanel->ctrlModeFFD->setEnabled(true);
            ...
            case mercury_rviz::ControlPanel::control_modes::FEEDFORWARD:
            ...
            case mercury_rviz::ControlPanel::control_modes::TELEOP:
                ...
                if (teleopPID == 0) {
                    ...
                    char * command = "ros2";
                    char * commandArgList[] = {
                        "ros2",           "launch", "mercury_controller", "teleop.launch.py",
                        teleopLaunchArgC, nullptr};
            ...
            case mercury_rviz::ControlPanel::control_modes::DISABLED:
        ...
        if (uiPanel->CtrlSendCmd->isEnabled()) {
            if (diff.to_chrono<std::chrono::seconds>() > odom_timeout)
                RVIZ_COMMON_LOG_WARNING(
                    "ControlPanel: Odom timed out! disabling local control buttons");

            if (!vehicleEnabled)
                RVIZ_COMMON_LOG_WARNING(
                    "ControlPanel: vehicle disabled! disabling local control buttons");
        }
...
void ControlPanel::displayPopupWindow(
    const std::string & warningMessage, const std::string & text) {
    QMessageBox msgBox;
...
void ControlPanel::handleCommand(bool updateInteractiveMarker) {
    ...
    if (ctrlMode == mercury_rviz::ControlPanel::control_modes::POSITION ||
        ctrlMode == mercury_rviz::ControlPanel::control_modes::FEEDFORWARD) {
        ...
        // QMessageBox::warning(uiPanel->CtrlSendCmd, "Control mode not supported!", "Control mode
        // is not supported by the controller! It will be removed from the panel soon.");
    }
    ...
    auto linCmd = mercury_msgs::msg::ControllerCommand();
    ...
    auto angCmd = mercury_msgs::msg::ControllerCommand();
...
void ControlPanel::odomCallback(const nav_msgs::msg::Odometry & msg) {
    ...
    if (!transformBetweenFrames(
            msg.pose.pose, frame_coordinates, msg.header.frame_id, command_frame)) {
        return;
    }

    // parse the quaternion to RPY
    tf2::Quaternion q(
        frame_coordinates.orientation.x, frame_coordinates.orientation.y,
        ...
        if (msg.status[0].name.find("ekf") != std::string::npos) {
            ...
            for (int i = 0; i < (sizeof(msg.status[1].values) / 6); i++) {
                ...
                if (msg.status[1].values[i].key.find("Actual frequency") != std::string::npos) {
                    uiPanel->OdomDiagnostics->setText(
                        QString::fromStdString(msg.status[1].values[i].value));
                }
            }

        } else if (
            msg.status[0].name.find("Controller Status Values Length") != std::string::npos) {
...
void ControlPanel::sendKillMsgTimer() {
    auto killMsg = mercury_msgs::msg::KillSwitchReport();
    killMsg.kill_switch_id = mercury_msgs::msg::KillSwitchReport::KILL_SWITCH_RQT_CONTROLLER;
...
bool ControlPanel::transformBetweenFrames(
    ...
    try {
        transform = tf_buffer->lookupTransform(to_frame, from_frame, tf2::TimePointZero);
...
bool ControlPanel::getDesiredSetpointFromTextboxes(double results[6]) {
    ...
    return !(std::any_of(std::begin(convOk), std::end(convOk), [](bool i) { return !i; }));
}
 ...
 */
void ControlPanel::setptMarkerFeedback(
...
void ControlPanel::callTriggerService(rclcpp::Client<Trigger>::SharedPtr client) {
    ...
    QTimer::singleShot(250, [this, client]() { ControlPanel::waitForTriggerResponse(client); });
    auto node = getDisplayContext()->getRosNodeAbstraction().lock()->get_raw_node();
...
void ControlPanel::callSetBoolService(rclcpp::Client<SetBool>::SharedPtr client, bool value) {
    ...
    QTimer::singleShot(250, [this, client]() { ControlPanel::waitForSetBoolResponse(client); });
    auto node = getDisplayContext()->getRosNodeAbstraction().lock()->get_raw_node();
    clientSendTime = node->get_clock()->now();
}

void ControlPanel::callSetPoseService(
    rclcpp::Client<SetPose>::SharedPtr client, std::vector<double> pose) {
    // check to ensure service is availabl
    if (!client->wait_for_service(5s)) {
        return;
    }
    ...
    QTimer::singleShot(250, [this, client]() { ControlPanel::waitForSetPoseResponse(client); });
    auto node = getDisplayContext()->getRosNodeAbstraction().lock()->get_raw_node();
    clientSendTime = node->get_clock()->now();
}
...
PLUGINLIB_EXPORT_CLASS(mercury_rviz::ControlPanel, rviz_common::Panel);
