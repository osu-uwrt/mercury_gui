...
#define EXPECTED_CONTROLLER_DIAG_KEYS \
  5  //getting unreliable message size data, so this is hard coded :( - this should be the number of keys in the message not the number that should be accessed
...
void ControlPanel::onInitialize()
{
  ...
  connect(
    uiPanel->reloadController, &QPushButton::clicked, this, &ControlPanel::handleReloadController);

  //connect simulator apply
  connect(
    uiPanel->simulation_apply, &QPushButton::clicked, this, &ControlPanel::simulator_apply_clickec);
...
void ControlPanel::load(const rviz_common::Config & config)
{
  ...
  diagSub = node->create_subscription<diagnostic_msgs::msg::DiagnosticArray>(
    "/diagnostics", rclcpp::SystemDefaultsQoS(), std::bind(&ControlPanel::diagCallback, this, _1));
  ...
  setptServer =
    std::make_shared<interactive_markers::InteractiveMarkerServer>("interactive_setpoint", node);
  ...
  if (vehicleEnabled || override) {
    ...
    switch (ctrlMode) {
      case mercury_rviz::ControlPanel::control_modes::POSITION:
        ...
        setptServer->insert(
          interactiveSetpointMarker, std::bind(&ControlPanel::setptMarkerFeedback, this, _1));
      ...
      case mercury_rviz::ControlPanel::control_modes::TELEOP:
        ...
        if (teleopPID == 0) {
          ...
          char * command = "ros2";
          char * commandArgList[] = {
            "ros2", "launch", "mercury_controller", "teleop.launch.py", teleopLaunchArgC, nullptr};
...
void ControlPanel::handleCommand(bool updateInteractiveMarker)
{
  ...
  if (
    ctrlMode == mercury_rviz::ControlPanel::control_modes::POSITION ||
