...
void DiagnosticsPanel::diagCb(const diagnostic_msgs::msg::DiagnosticArray & msg) {
    ...
    try {
                ...
                for (size_t i = 0; i < pathComponents.size(); ++i) {
                    ...
                    if (parentItems.find(storagePath) == parentItems.end()) {
                        ...
                        QStandardItem * newItem =
                            new QStandardItem(QString::fromStdString(component));
        ...
        for (const auto & status : msg.status) {
            ...
            findMatchingItems(
                model->invisibleRootItem(), QString::fromStdString(statusName), matches);
            ...
            if (!matches.isEmpty()) {
                ...
                if (timerTick > 0) {
                    ...
                    // In a production environment, you might want to add new nodes to the existing
                    // tree instead
                    RVIZ_COMMON_LOG_INFO(
                        "DiagnosticsPanel: New diagnostic item found, refreshing tree");
                ...
                RVIZ_COMMON_LOG_WARNING(
                    "DiagnosticsPanel: Couldn't find item for status: " + statusName);
                continue;
            }
