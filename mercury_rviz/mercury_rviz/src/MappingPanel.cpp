...
void MappingPanel::load(const rviz_common::Config & config)
{
  ...
  ffcStopSvoClient = std::make_shared<GuiSrvClient<Trigger>>(
    node, robotNs + "/ffc/zed_node/stop_svo_rec", std::bind(&MappingPanel::setStatus, this, _1, _2),
  ...
  CalibGoalHandle::SharedPtr, const std::shared_ptr<const ModelFrame::Feedback> feedback)
{
  ...
  setStatus(
    tr("Calibrating map frame (%1/%2)...")
      .arg(QString::number(feedback->sample_count), QString::number(ui->calibProgress->maximum())),
    "000000");
}
