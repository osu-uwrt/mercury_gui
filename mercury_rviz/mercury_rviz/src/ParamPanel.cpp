...
std::vector<rclcpp::Parameter> ParamPanel::getParams() {
    ...
    try {
        rcl_interfaces::msg::ListParametersResult msg =
            param_client->list_parameters(prefixes, 1, 1s);
        ...
        switch (param.get_type()) {
            ...
            case rclcpp::PARAMETER_BOOL_ARRAY: {
                ...
                for (unsigned int i = 0; i < arr.size(); i++) {
                    ...
                    box->setProperty(
                        "orig", static_cast<bool>(arr.at(i))); // Store per element in array
                    connect(
                        box, &QCheckBox::stateChanged, this, &ParamPanel::parameterValueChanged);
...
void ParamPanel::apply() {
    ...
    for (int i = 0; i < this->paramLayout->count(); i++) {
        ...
        if (QWidget * widget = dynamic_cast<QWidget *>(containerLayout->itemAt(1)->widget())) {
            ...
            if (originalValues.contains(paramName) &&
                ...
                if (auto spinBox = qobject_cast<QSpinBox *>(valueWidget)) {
                    ...
                    modifiedParams.push_back(
                        rclcpp::Parameter(paramNameStr, doubleSpinBox->value()));
                    ...
                    modifiedParams.push_back(
                        rclcpp::Parameter(paramNameStr, checkBox->isChecked()));
            ...
            (arrLayout = dynamic_cast<QVBoxLayout *>(containerLayout->itemAt(1)->layout()))) {
            // Handle arrays
            ...
            switch (types[0]) {
                case rclcpp::PARAMETER_INTEGER_ARRAY: {
                    ...
                    for (int j = 0; j < arrLayout->count(); j++) {
                        QHBoxLayout * arrHBox =
                            dynamic_cast<QHBoxLayout *>(arrLayout->itemAt(j)->layout());
                        if (arrHBox && arrHBox->count() > 1) {
                            ...
                            if (box) {
                                ...
                                if (!orig.isValid() ||
                                    orig.toLongLong() != static_cast<qlonglong>(box->value())) {
                                    isModified = true;
                                }
                            }
                        }
                ...
                case rclcpp::PARAMETER_DOUBLE_ARRAY: {
                    ...
                    for (int j = 0; j < arrLayout->count(); j++) {
                        ...
                        if (arrHBox && arrHBox->count() > 1) {
                            QDoubleSpinBox * box =
                                dynamic_cast<QDoubleSpinBox *>(arrHBox->itemAt(1)->widget());
                            if (box) {
                                values.push_back(box->value());
                                QVariant orig = box->property("orig");
                ...
                case rclcpp::PARAMETER_BOOL_ARRAY: {
                    ...
                    for (int j = 0; j < arrLayout->count(); j++) {
                        ...
                        if (arrHBox && arrHBox->count() > 1) {
                            QCheckBox * box =
                                dynamic_cast<QCheckBox *>(arrHBox->itemAt(1)->widget());
                            if (box) {
                ...
                case rclcpp::PARAMETER_STRING_ARRAY: {
                    ...
                    for (int j = 0; j < arrLayout->count(); j++) {
                        ...
                        if (arrHBox && arrHBox->count() > 1) {
                            QTextEdit * box =
                                dynamic_cast<QTextEdit *>(arrHBox->itemAt(1)->widget());
                            if (box) {
            ...
            if (hasErrors) {
                ...
                if (!arrayParamsChanged.empty()) {
                    ...
                    for (int i = 0; i < this->paramLayout->count(); i++) {
                        ...
                        QLabel * label =
                            dynamic_cast<QLabel *>(containerLayout->itemAt(0)->widget());
                        if (!label) continue;
                        ...
                        if (auto * arrLayout =
                            ...
                            for (int j = 0; j < arrLayout->count(); j++) {
                                if (auto * arrHBox = dynamic_cast<QHBoxLayout *>(
                                        arrLayout->itemAt(j)->layout())) {
                                    if (arrHBox->count() > 1) {
                                        if (auto * sb = dynamic_cast<QSpinBox *>(
                                                arrHBox->itemAt(1)->widget())) {
                                            sb->setProperty(
                                                "orig", static_cast<qlonglong>(sb->value()));
                                        } else if (
                                            auto * dsb = dynamic_cast<QDoubleSpinBox *>(
                                                arrHBox->itemAt(1)->widget())) {
                                            ...
                                            auto * te = dynamic_cast<QTextEdit *>(
                                                arrHBox->itemAt(1)->widget())) {
                ...
                showStatusMessage(
                    QString("Successfully applied %1 modified parameters")
                        .arg(modifiedParams.size()),
                    3000);
            }
        } catch (const std::exception & e) {
