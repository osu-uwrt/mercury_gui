...
void DiagnosticOverlay::onInitialize() {
    ...
    zedSub = node->create_subscription<sensor_msgs::msg::Temperature>(
        zedTopic, rclcpp::SystemDefaultsQoS(),
        std::bind(&DiagnosticOverlay::zedCallback, this, _1));

    std::string leakTopic = robotNsProperty->getStdString() + "/state/leak";
    leakSub = node->create_subscription<std_msgs::msg::Bool>(
        leakTopic, rclcpp::SystemDefaultsQoS(),
        std::bind(&DiagnosticOverlay::leakCallback, this, _1));
        ...
        if (diagnostic.name == "/Robot Diagnostics") {
            ...
            if (found_temp) {
                ...
                double normalizedTemp =
                    std::min(std::max(max_temp, 0.0), 100.0) / 100.0; // 0-100 C range
                double newAngle =
                    90.0 - (normalizedTemp * angleRange); // Start at top and go clockwise
        ...
        if (!startedLeaking) {
            ...
            if (msgBox.clickedButton() == (QAbstractButton *)killButton) {
                RVIZ_COMMON_LOG_WARNING(
                    "DiagnosticOverlay: NOT sending battery kill message. This feature was "
                    "disabled");
            }
