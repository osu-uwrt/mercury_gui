#include "mercury_rviz/FeedforwardPanel.hpp"
...
FeedforwardPanel::~FeedforwardPanel() { delete ui; }
...
void FeedforwardPanel::load(const rviz_common::Config & config) {
    ...
    // we want a publisher to publish to controller/ff_body_force to command the feedforward of the
    // solver we also want a timer to periodcally publish that message using the publisher
...
void FeedforwardPanel::onInitialize() {
    connect(
        ui->XdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::XFunction);
    connect(
        ui->YdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::YFunction);
    connect(
        ui->ZdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::ZFunction);
    connect(
        ui->RdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::RFunction);
    connect(
        ui->PdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::PFunction);
    connect(
        ui->YAWdoubleSpinBox, qOverload<double>(&QDoubleSpinBox::valueChanged), this,
        &FeedforwardPanel::YAWFunction);
...
void FeedforwardPanel::timerCb() { ffForcePub->publish(msgToPublish); }

} // namespace mercury_rviz

#include <pluginlib/class_list_macros.hpp> // NOLINT
PLUGINLIB_EXPORT_CLASS(mercury_rviz::FeedforwardPanel, rviz_common::Panel);
